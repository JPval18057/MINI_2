...
void config_sensor(void){
    char temp = 0;
    SSPCON2bits.SEN = 1; //INICIAR SECUENCIA DE INICIO
    //ESPERAMOS A QUE SE ACTIVE LA BANDERA
    while (PIR1bits.SSPIF);
    PIR1bits.SSPIF = 0;//APAGAMOS LA BANDERA
    //DIRECCIÓN DEL SENSOR
    temp = sensor_dir & 0b11111110; //HACEMOS UN AND CON ESO PARA CAMBIAR ÚNICAMENTE EL ÚLTIMO BIT
    SSPBUF = temp; //MANDAMOS LA DIRECCIÓN
    //ESPERAMOS EL ACKNOWLEDGE
    while (!SSPCON2bits.ACKSTAT);
    //ACÁ YA LLAMAMOS LA FUNCIÓN PARA MANDAR DATOS
    return;
}
